...
// GET — every order currently stuck in 'error', most recent first.
...
// POST { queueId } — reset one failed row back to 'pending' so the next
// Web Connector poll retries it from scratch.
export async function POST(request: NextRequest) {
  ...
  if (row.status !== 'error') {
    return NextResponse.json({ error: `Row is '${row.status}', not 'error' — nothing to retry.` }, { status: 400 })
  }

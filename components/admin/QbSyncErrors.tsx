...
interface SyncError {
  ...
  // Set instead of the score when several customers share the buyer's email
  // or name — the candidate shown is one of N, not a near-miss.
  matchCandidateCount?: number | null
  updatedAt: string
}
      ...
      {needsReview.length > 0 && (
        <div className="mt-6 rounded-xl bg-white border border-blue-200 shadow-sm overflow-hidden">
          ...
          <p className="px-5 pb-3 text-xs text-gray-500 -mt-2">
            These orders stopped short of a confident match: either the buyer&apos;s name is similar but not
            identical to an existing QuickBooks customer, or several customers share their email or name
            (common with shared AP inboxes). Confirm which one is right, then either link to it or create a
            new customer.
          </p>
            ...
            {needsReview.map((e) => (
              <div key={e.queueId} className="px-5 py-3 flex items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  {typeof e.matchCandidateCount === 'number' && e.matchCandidateCount > 1 ? (
                    <p className="mt-0.5 text-xs text-blue-700">
                      <span className="font-semibold">{e.matchCandidateCount} QuickBooks customers</span> share this
                      buyer&apos;s email or name — pick the right one in Link Customers, e.g.{' '}
                      <span className="font-semibold">{e.matchCandidateName}</span>
                    </p>
                  ) : (
                    <p className="mt-0.5 text-xs text-blue-700">
                ...
                <div className="flex flex-shrink-0 flex-col gap-1.5 items-end">
                  {typeof e.matchCandidateCount === 'number' && e.matchCandidateCount > 1 ? (
                    // Several customers are tied — there is no single "the
                    // match" to accept, so send the admin to the picker
                    // rather than offering a button that would attach the
                    // order to an arbitrary one of them.
                    <a

...
- [project-woo-role-write-fix](project-woo-role-write-fix.md) — 2026-08-10: assign-woo-tier-roles.mjs --apply silently no-op'd (wc/v3 role field is read-only); fixed via new WP_ADMIN_APP_PASSWORD + wp/v2/users writes, 2 real customers now correctly on Retail role

...
- [project-erply-image-backfill](project-erply-image-backfill.md) — CONFIRMED 2026-08-03: Erply image API (read+write) now live via saveProductPicture, no longer gated; new Woo->Erply backfill direction needed for ~1,900 SKUs
- [project-erply-pagination-fix](project-erply-pagination-fix.md) — 3 real-data bugs fixed 2026-07-30; dry run found real sync would've wiped 1,028 image_urls + all 2,870 stock_qty (now excluded via skipFields, see below) — 143 deactivate-candidates still unreviewed, DO NOT enable sync yet
...
- [project-woo-price-integration-markup-bug](project-woo-price-integration-markup-bug.md) — UPDATED 2026-08-14: found the actual filter — `wwp_regular_price_change()` in Wholesale For WooCommerce's `class-wwp-wholesale-multiuser.php`, hooked on both get_price/get_regular_price at priority 200 — but it explicitly skips itself on REST requests, contradicting the observed API behavior; leading theory is a stale WooCommerce price transient, not yet tested (read-only investigation only, no site changes)

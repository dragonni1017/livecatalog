...
- [project-auth-redirect-allowlist](project-auth-redirect-allowlist.md) — 2026-09-09 RESOLVED: customer password reset was dead for everyone — live domain `lyusa.app` was never in Supabase's redirect allow-list, and GoTrue silently substitutes the Site URL instead of erroring. Fixed + verified. Preview/vercel.app URLs are now blocked by design; probe the allow-list with `admin.generateLink` whenever a redirect auth flow "does nothing"

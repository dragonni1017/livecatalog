...
- [project-woo-price-integration-markup-bug](project-woo-price-integration-markup-bug.md) — UPDATED 2026-08-14 (4 passes): Wholesale plugin's price filter, WooCommerce-core's RestApiCache trait (feature flag OFF), and Codisto/NetSuite sync plugins all ruled out by source+config reading; paradox unresolved, needs live plugin-deactivate test or a full-page-cache purge+recheck (read-only investigation only, no site changes)

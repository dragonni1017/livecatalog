...
- [project-rep-price-tier-and-qbwc-plan](project-rep-price-tier-and-qbwc-plan.md) — 2026-08-20/21: rep-tier % bug fixed+verified; QBWC hardware-tested (4/4 acked); 2026-08-21 added ship-to address + fixed a real PONumber-after-Memo qbXML ordering bug (both verified live), then built /admin/quickbooks/customers to pre-link existing QB customers by email before their first order syncs (avoids CustomerQueryRq's name-only-lookup duplicate risk) -- server-side verified, real-hardware pull still unconfirmed -- plan at C:\Users\Dragon\.claude\plans\synthetic-greeting-blum.md

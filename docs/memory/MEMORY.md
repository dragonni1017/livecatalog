...
- [project-customers-table-never-applied](project-customers-table-never-applied.md) — 2026-08-21: `customers` table (migration 0012) was written but never run; buyer-file discounts silently never worked since the feature shipped; applied live + RLS added (0036), verified working
- [project-order-fulfillment-stock-decrement](project-order-fulfillment-stock-decrement.md) — 2026-08-21: stock_qty now decrements on order Converted (was never decremented before); found+fixed a real double-decrement bug (converted->contacted->converted) live via `stock_decremented_at` (migration 0037), same pattern as entered_in_qb
- [project-rep-price-tier-and-qbwc-plan](project-rep-price-tier-and-qbwc-plan.md) — 2026-08-20/21: rep-tier % bug + QBWC hardware-tested/verified; ship-to address + PONumber ordering fix; /admin/quickbooks/customers pre-link tool. UPDATE 2026-08-31: customer-directory pull confirmed done (4,828 real customers); sync-error Retry UI + auto-revert-on-failure shipped; dropped-connection duplicate-send risk probed live -- no dup-send risk, but found+fixed a stuck-'sent'-row gap (new "Stuck syncs" panel) -- plan at C:\Users\Dragon\.claude\plans\synthetic-greeting-blum.md
- [project-product-categories-outage-20260821](project-product-categories-outage-20260821.md) — 2026-08-21: real multi-category support (product_categories join table) shipped, but migration 0038 caused a brief live "0 products" outage — RLS gap + PGRST201 ambiguous-embed, both fixed within minutes; see also the new CLAUDE.md rules on both patterns
- [project-qb-customer-matching](project-qb-customer-matching.md) — 2026-09-08: QBWC now matches an existing QB customer before auto-creating (migrations 0043/0044). READ BEFORE touching the QB customer path: an email does NOT uniquely identify a customer (187 shared, one on 64 records), QBWC progress=100 ends the session (broke the customer pull since it shipped), merging is QB-Desktop-only; ~15 duplicate pairs deliberately left alone
- [project-vercel-preview-live-supabase](project-vercel-preview-live-supabase.md) — 2026-09-08: Preview env vars added (every preview build had been failing); previews now read AND WRITE the live Supabase project — a preview URL is not a sandbox
- [project-admin-users-screen](project-admin-users-screen.md) — 2026-09-09: new `/admin/users` full account-management screen (customer+rep+admin), separate from staff-only accounts/route.ts; role-storage convention (delete key for customer, not literal string) + password-reset mechanism (server-side anon `resetPasswordForEmail`, not `generateLink`) documented here
- [project-auth-redirect-allowlist](project-auth-redirect-allowlist.md) — 2026-09-09 RESOLVED (verified on a phone): customer password reset was dead since the custom domain launched, via FOUR independent causes — unlisted redirect (GoTrue silently substitutes the Site URL), PKCE device-locking (@supabase/ssr hard-codes flowType AFTER spreading your options, so the first fix was inert), Titan refusing a sender it does not own (surfaces as a bare `{}`), and no page handling the grant. Read before touching any auth email flow
- [project-admin-fetch-error-sweep](project-admin-fetch-error-sweep.md) — 2026-09-09: admin components swept to `lib/admin-fetch.ts`'s `readApiError`/`TRANSPORT_ERROR`, replacing parse-before-check; `/admin/api/sync` POST uses `.reason` not `.error` (left unconverted) — only route found that doesn't fit the helper
- [project-product-measurements](project-product-measurements.md) — 2026-09-11: products gained case_* dims+weight (migration 0045) for Erply bin capacity, 2,352 rows backfilled, worklist+importer round trip verified; data is INCHES/POUNDS though WooCommerce declares kg/cm (2.2x/2.54x trap), figures are master-carton, per-piece is out of scope, Erply bins have NO dimension or weight-limit fields, 28 values are physically impossible, a metric-filled sheet is undetectable, id-only upserts can't update these columns, and upstream values drift between runs; 986 of 3,222 still need measuring
- [project-bin-capacity-type-grouping](project-bin-capacity-type-grouping.md) — 2026-09-14: all 518 bins now grouped into 8 bin_types (5 by shelf level + receiving/shipping/combined-aisle-floor) via assign-bin-types-by-level.mjs + assign-remaining-bin-types.mjs; ALL dimensions still null, no spec doc exists anywhere — blocked on a physical tape-measure pass via /admin/bins
- [project-packing-list-importer](project-packing-list-importer.md) — 2026-09-14: import-packing-list.mjs (dims, safe on any-age file, live-applied 27/27) + add-stock-from-packing-list.mjs (Erply stock, exact-SKU-only, tested but NOT applied — old shipment showed 0 stock, would've injected stale phantom stock); new/unmatched SKUs are report-only by design, no auto product creation
- [project-net-terms-approval-flow](project-net-terms-approval-flow.md) — 2026-09-15: /admin/credit-applications can now approve/decline (migration 0047 applied + write path verified live); decision feeds nothing downstream, and the 4 pending rows are real buyers — do not smoke-test on them
- [project-receiving-phase-1](project-receiving-phase-1.md) — 2026-09-16: /admin/receiving takes a container end to end in one pass (stock for existing SKUs + create new ones from the Commercial Invoice); 0048/0049 applied; real 2026 files use 总PCS not QTY; Erply CANNOT set a price via API and pricing is DECIDED as a manual step — do not re-probe
- [project-fake-stock-1000-hold](project-fake-stock-1000-hold.md) — 2026-09-17: CONFIRMED the fake 1000 lives in ERPLY, 1,881 of 3,076 active SKUs (61%), so no sync will clear it and receiving adds on top; STANDING: do NOT correct it (Dragon 2026-09-17); receiving a container on top of it is fine
- [project-suffix-barcode-collisions](project-suffix-barcode-collisions.md) — 2026-09-17: 52 suffixed SKUs carry their base SKU's barcode; 51 hidden + never ordered, F286411-M was the only visible variant (its base was already hidden, so no storefront hazard) and is now hidden too; the collision persists in Erply and Woo
- [project-containers-20260917](project-containers-20260917.md) — 2026-09-17: three arrival lists dry-run through receiving (parse-only); fixed two staging bugs (normalizeBarcode kept inner spaces, catalog lookup was case-sensitive); 68 of 92 SKUs are new and K229582 spans two containers, which the file_hash makes a one-shot ordering problem; the K229480 barcode edit it also describes was REVERTED 2026-09-18
- [feedback-barcode-length-not-truncation](feedback-barcode-length-not-truncation.md) — a short barcode is NOT damaged data on this account; length varies by setup era, a validating check digit proves nothing, never "repair" one
- [project-qb-item-pull](project-qb-item-pull.md) — 2026-09-18: QuickBooks Desktop is the source of truth for NEW product info; QBWC item pull built (migration 0050, /admin/quickbooks/items) to mirror its item list; ItemQueryRq returns a different Ret per item type and sub-items are Parent:Child; auto-fill of proposed_name NOT built yet
- [project-receiving-to-catalog-20260923](project-receiving-to-catalog-20260923.md) — 2026-09-23: five containers received (75 new Erply SKUs, stock verified); the Erply sync could NOT insert them (products_id_seq collides with hand-assigned prod-NNNNN ids, whole 500-row chunks fail — migration 0052 reseeds it; sequence verified ahead of max id 2026-09-25); all 75 pushed into the catalog hidden at $0 (a visible $0 product is orderable), 52 with photos; admin "Sync Now" was missing the category alias/skip the cron has
- [project-image-sizing-contract](project-image-sizing-contract.md) — 2026-09-23: stored image URLs are always RAW originals, every width is applied at render time by lib/image.ts — do NOT pre-compress before Cloudinary (1.95MB original delivers as 50KB at w_800, 4.6KB at w_150) and never bake a transform into the stored URL; fixed the product gallery serving full-size originals behind 56px thumbnails (eagerly preloaded) and SearchInput handing raw URLs to next/image
- [feedback-invoice-not-price-source](feedback-invoice-not-price-source.md) — the Commercial Invoice arrives AFTER receiving and is only for item addition/adjustment; never propose it as the source of intended prices — those come only from the new-products panel, and pre-2026-09-24 products stay NO INTENT (price them from the worklist)

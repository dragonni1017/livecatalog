// reformat-and-categorize-hair.mjs
// Run with: node scripts/reformat-and-categorize-hair.mjs
//
// Same treatment as the earlier batches, for the 4-row Hair Band batch
// (data/qbd-catalog-compare/hair-review-enriched.xlsx) -- a SEPARATE QB
// parent item from the earlier Headband batch, but the same real category.
//
// CATEGORY: groupID 31 "Hair Bands" for all 4 -- same real established
// category already confirmed live (twice: for the Keychains batch's
// HKHB103 reassignment and the Headband batch itself).
//
// NAME reformatting: "hairband" (one word, no space) is consolidated to
// "Hair Band" (two words) to match this batch's OWN dominant convention
// (3 of 4 rows already use the split form) -- the opposite direction from
// the Headband batch, which consolidated toward the single-word form to
// match ITS dominant convention. Each batch's spelling stays internally
// consistent with itself rather than forcing one global spelling across
// both, since "Hair Band" and "Headband" are legitimate separate synonyms
// that coexist in the real catalog, not a typo of each other.
//
// Read-only against the source file. Writes a NEW xlsx to
// data/qbd-catalog-compare/hair-review-final.xlsx.
...
const SOURCE_XLSX = path.join(ROOT, 'data', 'qbd-catalog-compare', 'hair-review-enriched.xlsx')
const OUT_XLSX = path.join(ROOT, 'data', 'qbd-catalog-compare', 'hair-review-final.xlsx')
...
const TYPO_FIXES = [[/\bhairband\b/gi, 'Hair Band']]
...
function main() {
  ...
  const rows = XLSX.utils.sheet_to_json(wb.Sheets['Hair Review'], { defval: null })
  ...
  XLSX.utils.book_append_sheet(wbOut, ws, 'Hair Final')

// reformat-and-categorize-fans.mjs
// Run with: node scripts/reformat-and-categorize-fans.mjs
//
// Same treatment as the earlier batches, for the 8-row Fans batch
// (data/qbd-catalog-compare/fans-review-enriched.xlsx).
//
// CATEGORY: groupID 19 "Fan" for all 8 -- same real established category
// already confirmed live in the Toys/Dolls batch (28/33 precedent).
//
...
// data/qbd-catalog-compare/fans-review-final.xlsx.
...
const SOURCE_XLSX = path.join(ROOT, 'data', 'qbd-catalog-compare', 'fans-review-enriched.xlsx')
const OUT_XLSX = path.join(ROOT, 'data', 'qbd-catalog-compare', 'fans-review-final.xlsx')
...
const GROUP = { id: 19, name: 'Fan' }

function main() {
  const wb = XLSX.readFile(SOURCE_XLSX)
  const rows = XLSX.utils.sheet_to_json(wb.Sheets['Fans Review'], { defval: null })
  ...
  XLSX.utils.book_append_sheet(wbOut, ws, 'Fans Final')

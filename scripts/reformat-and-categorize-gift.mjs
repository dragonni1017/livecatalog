// reformat-and-categorize-gift.mjs
// Run with: node scripts/reformat-and-categorize-gift.mjs
//
// Same treatment as the earlier batches, for the 11-row Gift Bags batch
// (data/qbd-catalog-compare/gift-review-enriched.xlsx).
//
// CATEGORY: groupID 30 "Gift Bags" for all 11 -- confirmed live
// (searchName "gift bag" -> 43/50 dominant precedent, real established
// category added 2026-06-03).
//
...
// data/qbd-catalog-compare/gift-review-final.xlsx.
...
const SOURCE_XLSX = path.join(ROOT, 'data', 'qbd-catalog-compare', 'gift-review-enriched.xlsx')
const OUT_XLSX = path.join(ROOT, 'data', 'qbd-catalog-compare', 'gift-review-final.xlsx')
...
const GROUP = { id: 30, name: 'Gift Bags' }

function main() {
  const wb = XLSX.readFile(SOURCE_XLSX)
  const rows = XLSX.utils.sheet_to_json(wb.Sheets['Gift Review'], { defval: null })
  ...
  XLSX.utils.book_append_sheet(wbOut, ws, 'Gift Final')

// reformat-and-categorize-squeeze.mjs
// Run with: node scripts/reformat-and-categorize-squeeze.mjs
//
// Same treatment as the earlier batches, for the 14-row Squeeze Ball batch
// (data/qbd-catalog-compare/squeeze-review-enriched.xlsx).
//
// CATEGORY: groupID 52 "Squishy / Slime" for all 14 -- already confirmed
// live for this exact product type in the Squishy and Toys/Cars batches
// (searchName "squeeze ball" -> 3/3 precedent). No sub-type mismatches in
// this batch -- every row is a genuine squeeze/bouncing ball.
//
// NAME reformatting: standard conservative dz-aware pack-spec handling and
// shout-case detection; no new typo fixes needed (this batch is
// unusually clean).
//
// Read-only against the source file. Writes a NEW xlsx to
// data/qbd-catalog-compare/squeeze-review-final.xlsx.
...
const SOURCE_XLSX = path.join(ROOT, 'data', 'qbd-catalog-compare', 'squeeze-review-enriched.xlsx')
const OUT_XLSX = path.join(ROOT, 'data', 'qbd-catalog-compare', 'squeeze-review-final.xlsx')
...
const GROUP = { id: 52, name: 'Squishy / Slime' }

function main() {
  const wb = XLSX.readFile(SOURCE_XLSX)
  const rows = XLSX.utils.sheet_to_json(wb.Sheets['Squeeze Review'], { defval: null })
  ...
  XLSX.utils.book_append_sheet(wbOut, ws, 'Squeeze Final')

// reformat-and-categorize-beannie.mjs
// Run with: node scripts/reformat-and-categorize-beannie.mjs
//
// Same treatment as the earlier batches, for the 4-row Beannie batch
// (data/qbd-catalog-compare/beannie-review-enriched.xlsx).
//
// CATEGORY: groupID 5 "Beanies" for all 4 -- confirmed live (searchName
// "beanie" -> 13/16 dominant precedent, real established category added
// 2026-06-03, 16 active products). The individual item names in this
// batch just say "Hat" generically, not "beanie" specifically, but the QB
// parent item for this whole batch is literally "Beannie" (a typo for
// "Beanie") -- routed by department intent, not just a literal word
// match. groupID 32 "Hats" is ALSO real (added 2026-06-03, not a Sept-1
// duplicate) but much smaller (3 active products) -- Beanies is the
// better-established fit given the QB category context.
//
...
// data/qbd-catalog-compare/beannie-review-final.xlsx.
...
const SOURCE_XLSX = path.join(ROOT, 'data', 'qbd-catalog-compare', 'beannie-review-enriched.xlsx')
const OUT_XLSX = path.join(ROOT, 'data', 'qbd-catalog-compare', 'beannie-review-final.xlsx')
...
const GROUP = { id: 5, name: 'Beanies' }

function main() {
  const wb = XLSX.readFile(SOURCE_XLSX)
  const rows = XLSX.utils.sheet_to_json(wb.Sheets['Beannie Review'], { defval: null })
  ...
  XLSX.utils.book_append_sheet(wbOut, ws, 'Beannie Final')
